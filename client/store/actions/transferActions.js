...
export function getTransfers(id) {
  ...
  return async dispatch => {
    await axios
      .get(`http://192.168.1.114:3001/transfers/get?id=${id}`)
      .then(response => {
...
export function makeTransfer(data) {
  return async (dispatch) => {
    await axios
      .post(`http://192.168.1.114:3001/transfers/create?`,data);
    await axios
      .get(`http://192.168.1.114:3001/transfers/get?id=${data.origen}`)
      .then((response) => {
        dispatch({ type: GET_TRANSFERS, payload: response.data });
      })
      .catch((error) => console.log(error));
